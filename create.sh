...
function create {
  ...
  case "$LANG" in
	java)
		...
		CONFIG="--artifact-version $VERSION --api-package $PKG.api --invoker-package $PKG --model-package $PKG.model --artifact-id $NAME --group-id $GROUP --library retrofit2 -DhideGenerationTimestamp=true"
		;;
	ruby)
